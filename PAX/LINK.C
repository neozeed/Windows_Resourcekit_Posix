...
#endif
#if 0
    abase = LINKHASH(asb->sb_ino);
    (void) fprintf(stderr, "link.c/linkfrom - asb->sb_ino: %lu; LINKHASH(~): %p; *~: %p\n",
	(unsigned long) asb->sb_ino, abase, *abase);
    (void) fflush(stderr);
#endif /* Xn 1993-06-21 */
    for (linkp = *(abase = LINKHASH(asb->sb_ino)); linkp; linkp = linknext) {
#if 0
	(void) fprintf(stderr, "link.c/linkfrom - linkp: %p; linkp->l_nlink: %lu\n", linkp, (unsigned long) linkp->l_nlink);
	(void) fflush(stderr);
#endif /* Xn 1993-06-21 */
	if (linkp->l_nlink == 0) {
	    if (linkp->l_name) {
	    ...
	    if ((linknext = linkp->l_forw) != NULL) {  /* Xn */
	    ...
	    *abase = (Link *)NULL;
...
#if 0
		 (void) fprintf(stderr, "link.c/linkfrom - path: %p; path->p_name: \"%s\"\n", path, path->p_name);
		 (void) fflush(stderr);
#endif /* Xn 1993-06-21 */
		if (strcmp(path->p_name, name) == 0) {
		    ...
		    if ((pathnext = path->p_forw) != NULL) {  /* Xn */
...
printf("DF_TRACE_DEBUG: 		() in link.c\n");
	    ...
	    if ((path->p_forw = linkp->l_path) != NULL) {  /* Xn */
...
#if 0
	(void) fprintf(stderr, "link.c/linkto - asb->sb_mode & S_IFMT: %lo; S_IFDIR: %lo; linkp: %p\n",
	    (unsigned long) (asb->sb_mode & S_IFMT), (unsigned long) S_IFDIR, linkp);
	(void) fflush(stderr);
#endif /* Xn 1993-06-21 */
	return ((Link *)NULL);
    }
#if 0
    (void) fprintf(stderr, "link.c/linkto - filling in linkp\n");
    (void) fflush(stderr);
#endif /* Xn 1993-06-21 */
    ...
    if ((linkp->l_forw = *(abase = LINKHASH(asb->sb_ino))) != NULL) {  /* Xn */
