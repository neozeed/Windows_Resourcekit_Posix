...
printf("DF_TRACE_DEBUG: void read_archive() in extract.c\n");
    ...
    name[0] = '\0';
#if 0
    while ((header = get_header(name, &sb)) == 0) {
	(void) fprintf(stderr, "extract.c/read_archive - get_header(\"%s\"): 0\n", name);
	(void) fflush(stderr);
#else
...
#if 0
	    (void) fprintf(stderr, "extract.c/read_archive - before buf_skip\n");
	    (void) fflush(stderr);
#endif /* Xn 1993-06-22 */
	    if (((ar_format == TAR) 
...
#if 0
	    (void) fprintf(stderr, "extract.c/read_archive - after buf_skip\n");
	    (void) fflush(stderr);
#endif /* Xn 1993-06-22 */
	} else if (match) {
...
#if 0
    (void) fprintf(stderr, "extract.c/read_archive - get_header(\"%s\"): %d\n", name, header);
    (void) fflush(stderr);
#endif /* Xn 1993-06-22 */
    close_archive();
}
...
#if 0
	(void) fprintf(stderr, "extract.c/readtar - name: \"%s\"; status: %d\n", name, status);
	(void) fflush(stderr);
#endif /* Xn 1993-06-22 */
	switch (status) {
	case 1:		/* Valid header */
...
printf("DF_TRACE_DEBUG: static int inswab() in extract.c\n");
    ...
    if ((namesize = (ushort)SWAB(binary.b_name)) == 0 || namesize >= PATH_MAX) {  /* Xn */
