...
#define myuid	( (int)my_uid < 0? (my_uid = getuid()): my_uid )  /* Xn */
#define	mygid	( (int)my_gid < 0? (my_gid = getgid()): my_gid )  /* Xn */


/* Internal Identifiers */

#ifdef _POSIX_SOURCE  /* Xn */
static uid_t    saveuid = (uid_t)-993;  /* Xn */
static char     saveuname[TUNMLEN];  /* Xn */
static uid_t    my_uid = (uid_t)-993;  /* Xn */
  /* Xn */
static gid_t    savegid = (gid_t)-993;  /* Xn */
static char     savegname[TGNMLEN];  /* Xn */
static gid_t    my_gid = (gid_t)-993;  /* Xn */
#else  /* Xn */
...
#endif
#if 0 && WIN_NT
    printf("names.c/finduid - myuid: %lu\n", (unsigned long) myuid);
#endif
#if 1 && WIN_NT
    if (uname == NULL)
    {
...
#endif
#if 0
    (void) fprintf(stderr, "names.c/findgid - gname: \"%s\"; savegname: \"%s\"\n", gname, savegname);
    (void) fflush(stderr);
#endif /* Xn 1993-06-22 */
    if (gname[0] != savegname[0] || strncmp(gname, savegname, TUNMLEN) != 0) {
	strncpy(savegname, gname, TUNMLEN);
#if 0
	(void) fprintf(stderr, "names.c/findgid - before getgrnam; gname: \"%s\"\n", gname);
	(void) fflush(stderr);
#endif /* Xn 1993-06-22 */
	gr = getgrnam(gname);
#if 0
	(void) fprintf(stderr, "names.c/findgid - after getgrnam; gr: %p\n", gr);
	(void) fflush(stderr);
#endif /* Xn 1993-06-22 */
	if (gr) {
	    savegid = gr->gr_gid;
