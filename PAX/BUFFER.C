...
printf("DF_TRACE_DEBUG: void outdata() in buffer.c\n");
#endif
    if ((pad = (size % BLOCKSIZE)) != 0) {  /* Xn */
	pad = (BLOCKSIZE - pad);
    }
    while (size) {
	avail = buf_out_avail(&buf);
#if 0 && WIN_NT
	printf("buffer.c/outdata - avail: %u; size: %ld\n", avail, (signed long) size);
#endif
	size -= (OFFSET)(chunk = size < (OFFSET)avail ? (uint) size : avail);  /* Xn */
#if 0 && WIN_NT
	printf("buffer.c/outdata - chunk: %u\n", chunk);
#endif
#if 0 && 1 && WIN_NT
	memset(buf, 0, chunk);
#endif
...
#if 0 && WIN_NT
	    saved_errno = errno;
	    printf("buffer.c/outdata - before warn - sizeof buf: %u; ", (unsigned int) sizeof buf);
	    printf("oops: %d; got: %d; fd: %d; buf: \"%s\"; errno: %d; chunk: %u\n", oops, got, fd, buf, saved_errno, chunk);
#endif
	    warn(name, strerror(errno));  /* Xn */
#if 0 && WIN_NT
	    puts("buffer.c/outdata - after warn");
#endif
	    got = 0;
	}
	...
	if ((uint)got < chunk) {  /* Xn */
	    if (oops == 0) {
		oops = -1;
	    }
	    warn(name, "Early EOF");
	    while ((uint)got < chunk) {  /* Xn */
		buf[got++] = '\0';
	    }
	}
	buf_use(chunk);
...
printf("DF_TRACE_DEBUG: void write_eot() in buffer.c\n");
	...
	if ((pad = (total + M_STRLEN + H_STRLEN + TRAILZ) % BLOCKSIZE) != (OFFSET)0) {  /* Xn */
	    pad = BLOCKSIZE - pad;
	}
...
#endif
    while ((want = endx - idx) != 0) {  /* Xn */
	if (bufend - bufidx < 0) {
	    fatal("Buffer overflow in out_write\n");  /* Xn */
	}
#if 0 && WIN_NT
	puts("buffer.c/outwrite - before outflush");
#endif
	if ((have = bufend - bufidx) == 0) {
	    outflush();
#if 0 && WIN_NT
	puts("buffer.c/outwrite - after outflush");
#endif
	}
	if (have > want) {
	    have = want;
...
#if 0 && WIN_NT
	puts("buffer.c/outwrite - after memcpy");
#endif
	bufidx += have;
	idx += have;
	total += have;
...
*/
	...
	while (!failed && !areof && (uint)(bufstart + blocksize - bufend) >= blocksize) {  /* Xn */
	    if ((got = read(archivefd, bufend, (unsigned int) blocksize)) > 0) {
    		bufend += got;
#ifdef _POSIX_SOURCE                /* 7/3/90-JPB */
    		if (got != (int)blocksize)  /* Xn */
        		++areof;
#endif
	    } else if (got < 0) {
		failed = -1;
	...
	failed = 0;
	for (got = 0; got < (int)blocksize; ++got) {  /* Xn */
	    *bufend++ = '\0';
	}
	return (-1);
    }
...
printf("DF_TRACE_DEBUG: int ar_write() in buffer.c\n");
	...
	if (*bidx++) {
#ifdef _POSIX_SOURCE  /* Xn */
	    return (write(fd, buf, len) == (ssize_t)len ? 0 : -1);  /* Xn */
#else  /* Xn */
	    return (write(fd, buf, len) == (int)len ? 0 : -1);  /* Xn */
#endif  /* Xn */
	}
    }
...
printf("DF_TRACE_DEBUG: static uint buf_out_avail() in buffer.c\n");
#endif
#if 0 && WIN_NT
    printf("buffer.c/buf_out_avail - bufend: %p; bufidx: %p\n", bufend, bufidx);
#endif
    if (bufend - bufidx < 0) {
#if 0 && WIN_NT
	puts("buffer.c/buf_out_avail - before fatal");
#endif
    ...
    *bufp = bufidx;
#if 0 && WIN_NT
    printf("buffer.c/buf_out_avail - have: %d\n", have);
#endif
    return (have);
}
