...
printf("DF_TRACE_DEBUG: regexp *regcomp() in regexp.c\n");
	...
	if (flags & SPSTART) {
		...
		if (OP(scan) == EXACTLY && (int)strlen(OPERAND(scan)) >= len) {  /* Xn */
...
printf("DF_TRACE_DEBUG: static char *reg() in regexp.c\n");
	...
	parno = regnpar;
	regnpar++;
	ret = regnode((char)(OPEN + parno));  /* Xn */
    } else
	ret = (char *)NULL;
	...
	*flagp |= flags & SPSTART;
    ...
    ender = regnode((char)((paren) ? CLOSE + parno : END));  /* Xn */
    regtail(ret, ender);
...
#endif  /* Xn */
{
	...
	offset = val - scan;
    *(scan + 1) = (char)((offset >> 8) & 0377);  /* Xn */
    *(scan + 2) = (char)(offset & 0377);  /* Xn */
}
